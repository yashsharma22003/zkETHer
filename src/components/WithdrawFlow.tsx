...
export default function WithdrawFlow({ onClose }: WithdrawFlowProps) {
  ...
  const loadNotes = async () => {
    try {
      ...
      // Add demo note for Bob if no real notes exist
      const allNotes = notes.length > 0 ? notes : [demoNote];
      setRealNotes(allNotes);
      
      if (allNotes.length > 0 && !selectedNote) {
        setSelectedNote(allNotes[0]);
        console.log('🎯 Auto-selected first available note:', allNotes[0].commitment.slice(0, 10) + '...');
      }
      
      console.log(`📝 Loaded ${allNotes.length} available notes for withdrawal`);
    } catch (error) {
      console.error('❌ Failed to load notes:', error);
      // Fallback to demo note on error
      setRealNotes([demoNote]);
  ...
  // Demo note for Bob (1 USDC)
  const demoNote: StoredNote = {
    id: 'demo_bob_note',
    commitment: '0x1457301813474130abcdef1234567890abcdef1234567890abcdef1234567890',
    secret: '0xsecret456',
          ...
          if (prev >= 100) {
            ...
            setTimeout(() => {
              ...
              setTimeout(async () => {
                ...
                // Remove the demo note from the list after withdrawal
                if (selectedNote.id === 'demo_bob_note') {
                  setRealNotes([]);
                  console.log('🗑️ Demo note removed after withdrawal');
                }
            ...
            <>
              ...
              <View style={styles.content}>
                ...
                <View style={styles.centerSection}>
                  ...
                  <Text style={styles.withdrawAmount}>Amount: {selectedNote?.amount}</Text>
            ...
            <View style={styles.content}>
              <View style={styles.centerSection}>
                ...
                <Card style={styles.completeCard}>
                  <CardContent style={styles.completeCardContent}>
                    ...
                    <View style={styles.completeDetails}>
                      <Text style={styles.completeAmount}>{selectedNote?.amount} Withdrawn</Text>
                ...
                <View style={styles.balanceUpdate}>
                  ...
                  <Text style={styles.balanceAmount}>0 USDC UNLINKABLE</Text>
                  <Text style={styles.balanceNote}>(Note withdrawn and removed)</Text>
                </View>
